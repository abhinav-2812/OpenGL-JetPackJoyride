...
Specialcoin::Specialcoin(float x, float y, color_t color) {
    ...
    vertex_buffer_data[3] = 0.3f;
    vertex_buffer_data[4] = 0.0f;
    vertex_buffer_data[5] = 0.0f;
    vertex_buffer_data[6] = 0.3f * cos(ang * PI / 180.0);
    vertex_buffer_data[7] = 0.3f * sin(ang * PI / 180.0);
    ...
    this->object = create3DObject(GL_TRIANGLES, 9 * n * 3, vertex_buffer_data,
                                  color, GL_FILL);
}

void Specialcoin::draw(glm::mat4 VP) {
...
void Specialcoin::tick() { this->rotation += this->speed; }